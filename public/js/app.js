...
let mainPurple = window
  .getComputedStyle(document.documentElement)
  .getPropertyValue("--main-purple");
let mathRed = window
  .getComputedStyle(document.documentElement)
  .getPropertyValue("--math-red");
let scienceGreen = window
  .getComputedStyle(document.documentElement)
  .getPropertyValue("--science-green");
let englishBlue = window
  .getComputedStyle(document.documentElement)
  .getPropertyValue("--english-blue");
let historyYellow = window
  .getComputedStyle(document.documentElement)
  .getPropertyValue("--history-yellow");
let foreignOrange = window
  .getComputedStyle(document.documentElement)
  .getPropertyValue("--foreign-orange");
var inBrowser = 0;
var inSubForum = 0;
var inFeedItem = 0;
var inFeed = 1;
...
function generateFeed() {
  for (var i = 0; i < Object.keys(feedItems).length; i++) {
    ...
    feedItemRepliesContainer.className =
      "feedItemRepliesContainer shortReplies";
    ...
    for (var key in feedItems) {
      if (key == "feedItem" + i) {
        ...
        $(feedUserText).html(
          `<i class="fas fa-user"></i> ${feedItems[key].user} • ${
            feedItems[key].date
          }`
        );
        ...
        for (var x in feedItems[key].comments) {
          ...
          $(feedItemReplyUser).html(
            `<i class="fas fa-user"></i> ${feedItems[key].comments[x].user} • ${
              feedItems[key].comments[x].date
            }`
          );
  ...
  for (var i = 1; i <= Object.keys(subForumObject).length; i++) {
    ...
    for (var key in subForumObject) {
      if (key == "post" + (i - 1)) {
        ...
        $(supPostUserText).html(
          `<i class="fas fa-user"></i> ${subForumObject[key].user} • ${
            subForumObject[key].date
          }`
        );
        $(subPostCommentCountText).html(
          `<i class="fas fa-comment-alt"></i> ${
            subForumObject[key].commentCount
          } comments`
        );
        ...
        for (var x in subForumObject[key].comments) {
          ...
          $(subPostItemReplyUser).html(
            `<i class="fas fa-user"></i> ${
              subForumObject[key].comments[x].user
            } • ${subForumObject[key].comments[x].date}`
          );
...
$(document).on("click", ".requesting", function() {
  if (inFeedItem != 1) {
    ...
    $(this)
      ...
      .removeClass("shortDesc");
    $(this)
      .find(".feedDescription")
      .addClass("longDesc");
    $(this)
      .find(".feedItemRepliesContainer")
      .clone()
      .appendTo("#individualFeedItem");
    $("#individualFeedItem")
      .find(".feedItemRepliesContainer")
      .removeClass("shortReplies");
    $("#individualFeedItem")
      .find(".feedItemRepliesContainer")
      .addClass("longReplies");
    $(this)
      .clone()
      .prependTo("#individualFeedItem");
    $("#feedContainer").hide();
...
function postRequest() {
  var typeSelected =
    radioProvide.checked == true || radioRequest.checked == true;
  var descriptionInputted =
    document.getElementById("formDescriptionInput").value != "";
  var imageSelected =
    document.getElementById("displayImg").getAttribute("src") != "";
  if (!typeSelected) {
